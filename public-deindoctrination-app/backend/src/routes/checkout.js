...
router.post('/stripe/:itemId', authenticate, async function(req, res) {
  try {
    ...
    var session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: (process.env.FRONTEND_URL || 'http://localhost:5173') + '?purchase=success&item=' + req.params.itemId,
...
router.post('/paypal/:itemId', authenticate, async function(req, res) {
  try {
    ...
    request.requestBody({
      ...
      application_context: {
        return_url: (process.env.FRONTEND_URL || 'http://localhost:5173') + '?purchase=paypal_success',
        cancel_url: (process.env.FRONTEND_URL || 'http://localhost:5173') + '?purchase=cancelled',
  ...
  try {
    ...
    if (method === 'paypal') {
      ...
      request.requestBody({
        ...
        application_context: { return_url: (process.env.FRONTEND_URL || 'http://localhost:5173') + '?purchase=paypal_success', cancel_url: (process.env.FRONTEND_URL || 'http://localhost:5173') + '?purchase=cancelled', brand_name: 'Space Out', user_action: 'PAY_NOW' }
      });
      var order = await paypalClient.execute(request);
      var approveLink = order.result.links.find(function(l) { return l.rel === 'approve'; });
