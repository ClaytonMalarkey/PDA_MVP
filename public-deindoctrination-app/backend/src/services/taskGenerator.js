...
// === TASK DNA DIMENSIONS ===
// These multiply the combinatorial space massively
const INTENSITIES = [
  { name:'Beginner', xpMul:0.5, currMul:0.5, prefix:'Intro: ', cooldownMul:0.25 },
  { name:'Easy',     xpMul:0.8, currMul:0.8, prefix:'', cooldownMul:0.5 },
  { name:'Medium',   xpMul:1.0, currMul:1.0, prefix:'', cooldownMul:1.0 },
  { name:'Hard',     xpMul:1.5, currMul:1.5, prefix:'Challenge: ', cooldownMul:1.5 },
  { name:'Extreme',  xpMul:2.5, currMul:2.0, prefix:'Extreme: ', cooldownMul:2.0 },
];

const CONTEXTS = [
  { name:'Solo',        suffix:' — on your own', socialBonus:0 },
  { name:'Group',       suffix:' — with at least 2 others', socialBonus:0.2 },
  { name:'Competitive', suffix:' — compete with someone', socialBonus:0.3 },
  { name:'Teaching',    suffix:' — then teach it to someone', socialBonus:0.25 },
  { name:'Public',      suffix:' — share your result publicly', socialBonus:0.15 },
];

const ENVIRONMENTS = [
  { name:'Home',    prefix:'At home: ' },
  { name:'Outside', prefix:'Outdoors: ' },
  { name:'Online',  prefix:'Online: ' },
  { name:'Work',    prefix:'At work: ' },
  { name:'Any',     prefix:'' },
];

const DURATIONS = [
  { name:'5 min',   xpMul:0.3, cooldown:300000 },
  { name:'15 min',  xpMul:0.6, cooldown:900000 },
  { name:'30 min',  xpMul:1.0, cooldown:1800000 },
  { name:'1 hour',  xpMul:1.5, cooldown:3600000 },
  { name:'2 hours', xpMul:2.0, cooldown:7200000 },
  { name:'1 day',   xpMul:4.0, cooldown:86400000 },
  { name:'1 week',  xpMul:10.0, cooldown:604800000 },
];
...
function generateTask(seed) {
  ...
  return { taskId:'gen-'+seed, title, description, category:domain.name, categoryIcon:domain.icon,
    xpReward:xp, currencyReward:curr, cooldown:tier.cooldown, tier:tier.name,
    realReward, isActive:true, requiresVerification:ti>=3, isGenerated:true };
}

// === PERSONALIZED TASK GENERATOR ===
// Generates tasks tailored to a player's level, dominant domain, and history
function generatePersonalizedBatch(playerProfile, count) {
  count = count || 20;
  const { level = 1, dominantDomain = null, streak = 0, totalTasksCompleted = 0 } = playerProfile || {};

  // Determine appropriate tier based on level
  const tierBias = Math.min(Math.floor(level / 5), TIERS.length - 1);

  const dk = Object.keys(DOMAINS);
  const tasks = [];
  const used = new Set();

  // 40% tasks from dominant domain if set
  const domainBias = dominantDomain ? dk.findIndex(k => DOMAINS[k].name === dominantDomain) : -1;
  ...
  while (tasks.length < count && attempts < count * 5) {
    ...
    // Bias toward dominant domain
    if (domainBias >= 0 && Math.random() < 0.4) {
      seed = domainBias + dk.length * Math.floor(Math.random() * 100000);
    } else {
    ...
    const task = generateTask(seed);

    // Adjust tier to match player level
    const tierIdx = Math.min(tierBias + Math.floor(Math.random() * 2), TIERS.length - 1);
    const tier = TIERS[tierIdx];
    const r = hash(seed * 47 + 5);
    task.xpReward = tier.xpRange[0] + Math.floor(r * (tier.xpRange[1] - tier.xpRange[0]));
    task.currencyReward = tier.currRange[0] + Math.floor(r * (tier.currRange[1] - tier.currRange[0]));
    task.tier = tier.name;
    task.cooldown = tier.cooldown;

    // Streak bonus
    if (streak >= 7) {
      task.xpReward = Math.floor(task.xpReward * 1.2);
      task.streakBonus = true;
    }
...
// Generates 3 daily quests that reset every 24 hours
function generateDailyQuests(userId, date) {
  const dateStr = date || new Date().toISOString().split('T')[0];
  const baseSeed = userId ? userId.split('').reduce((a, c) => a + c.charCodeAt(0), 0) : 42;
  const dateSeed = dateStr.split('-').reduce((a, c) => a + parseInt(c), 0);
  const combined = baseSeed + dateSeed;

  const quests = [];
  const dk = Object.keys(DOMAINS);

  for (let i = 0; i < 3; i++) {
    const seed = (combined * (i + 1) * 7919) % 1000000;
    const task = generateTask(seed);

    // Daily quests are always Medium tier
    const tier = TIERS[2]; // Medium
    const r = hash(seed * 47 + 5);
    task.xpReward = tier.xpRange[0] + Math.floor(r * (tier.xpRange[1] - tier.xpRange[0]));
    task.currencyReward = tier.currRange[0] + Math.floor(r * (tier.currRange[1] - tier.currRange[0]));
    task.tier = 'Daily';
    task.isDaily = true;
    task.expiresAt = new Date(dateStr + 'T23:59:59Z').toISOString();
    task.dailyBonus = { xp: 50, currency: 25 }; // Bonus for completing all 3
...
// === CHALLENGE TASK GENERATOR ===
// Generates a hard weekly challenge
function generateWeeklyChallenge(userId) {
  const weekNum = Math.floor(Date.now() / (7 * 24 * 60 * 60 * 1000));
  const baseSeed = userId ? userId.split('').reduce((a, c) => a + c.charCodeAt(0), 0) : 42;
  const seed = (baseSeed + weekNum * 31337) % 1000000;
  const task = generateTask(seed);

  // Weekly challenge is always Mega tier
  const tier = TIERS[4];
  const r = hash(seed * 47 + 5);
  task.xpReward = tier.xpRange[0] + Math.floor(r * (tier.xpRange[1] - tier.xpRange[0]));
  task.currencyReward = tier.currRange[0] + Math.floor(r * (tier.currRange[1] - tier.currRange[0]));
  ...
  task.weeklyBonus = { xp: 500, currency: 250, legacyStones: 1 };
...
module.exports = {
  generateTask, generateRandomBatch, generateByDomain,
  generatePersonalizedBatch, generateDailyQuests, generateWeeklyChallenge,
  getDomainInfo, getTotalTaskCount,
  DOMAINS, TIERS, INTENSITIES, CONTEXTS, ENVIRONMENTS, DURATIONS
};
